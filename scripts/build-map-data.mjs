...
// Curated single-axis cluster labels, keyed by the deterministic TF-IDF term
// signature so they re-attach across rebuilds as long as a cluster's content is
// unchanged. Generated by scripts/label-clusters.mjs (LLM, one call over all
// clusters); see that file to regenerate. A cluster with no cache entry falls
// back to the single distinctive word below, so the build never needs the net.
const LABELS_PATH = path.join(path.dirname(fileURLToPath(import.meta.url)), "cluster-labels.json");
let labelCache = {};
try {
  labelCache = JSON.parse(fs.readFileSync(LABELS_PATH, "utf8"));
  console.log(`  loaded ${Object.keys(labelCache).filter((k) => k[0] !== "_").length} curated cluster labels`);
} catch {
  console.log("  no cluster-labels.json cache (using single-word fallback labels)");
}
// Signature = the community's top-3 distinctive terms, sorted so a pure
// reordering across rebuilds still hits the same cache entry.
const sigOf = (rankedTermList) =>
  rankedTermList.slice(0, 3).map(([t]) => t).sort().join(" / ");
...
ordered.forEach((c, idx) => {
  ...
  // Curated single-axis label (+ sublabel) keyed by TF-IDF signature; falls
  // back to the single distinctive word (labelFor) when a cluster isn't cached.
  const ranked = rankedTerms.get(c) || [];
  const termHints = ranked.slice(0, 6).map(([t]) => t);
  const signature = isOther ? "other / unclustered" : sigOf(ranked);
  const curated = labelCache[signature];
  const label = isOther ? "Other" : (curated?.label ?? labelFor.get(c));
  const sublabel = curated?.sublabel ?? "";
  ...
  communityMeta.set(c, {
    ...
    label, sublabel, signature, term_hints: termHints,
    color: isOther ? OTHER_COLOR : PALETTE[idx % PALETTE.length],
