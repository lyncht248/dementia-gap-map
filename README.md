...
- **Switch framing** with the *Disease areas · Flywheel* toggle. **Flywheel** reframes the 8 mechanistic "Alzheimer's cure" bets (amyloid, tau, lipid/APOE, microglia, endocytosis, synaptic, vascular, epigenetic) as a development pipeline — Research → Genetics → Models → Trials → Results — with a circular dot cluster per hypothesis×stage. Each dot is one item; hovering traces its lineage across stages (paper → gene → trial and back), and switching in morphs the paper dots over from the semantic map.
